...
function getplayerrow(player) {
  ...
  if (player) {
    ...
    playerRow = `<tr>
          ...
          <td class="align-middle">${player.name_op}</td>
...
function getdb_op() {
  ...
  $.ajax({
    ...
    success: function (rows) {
      ...
      if (rows.db_op) {
        var db_oplist = "";
        $.each(rows.db_op, function (index, player) {
          db_oplist += getplayerrow(player);
        });
        $("#userstable tbody").html(db_oplist);
        let totaldb_op = rows.count;
        let totalpages = Math.ceil(parseInt(totaldb_op) / 4);
...
$(document).ready(function () {
  // add/edit user
  $(document).on("submit", "#addop", function (event) {
        ...
        if (player) {
          ...
          const profile = `<div class="row">
                ...
                <div class="col-sm-6 col-md-8">
                  <h4 class="text-primary">${player.name_op}</h4>
  ...
  $("#searchinput").on("keyup", function () {
    ...
    if (searchText.length > 1) {
      $.ajax({
        ...
        success: function (db_op) {
          if (db_op) {
            var db_oplist = "";
            $.each(db_op, function (index, player) {
              db_oplist += getplayerrow(player);
            });
            $("#userstable tbody").html(db_oplist);
            $("#pagination").hide();
